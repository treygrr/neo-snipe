...
| File | What it does |
|---|---|
| `release.mjs` | `npm run release`. Wipes `release/`, runs the Chrome `build`, copies `dist/` to `release/chrome/` with the inlined `README` template as its `README.md`, then zips it. Releases are Chrome only and run no tests — they are run by hand before a release. |
...
- `release.mjs` warns instead of failing when the `zip` binary is missing.
