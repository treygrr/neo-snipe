...
| `SettingsView.vue` | The settings view, opened by the bar's cog button. Five groups, each a short title with a fuller description under it, every control wired to `setSetting`: **App settings** (Detect Premium `premiumAuto`, I have Premium `premium`, Hover badges `hoverOnly`, the Badge size stepper (`.ns-set-size--badge`: 16–32px in 4px steps, `badgeIconStep`), Track dailies `trackDailyVisits`, Buy margin `minMargin`); **Layout** (a hint that the bar's buttons, the panel's title bar and the popover's tabs always drag; Vertical mode `verticalLauncher`, the icon size stepper (`.ns-set-size--bar`: −, the size in px, +; 20–36px in 4px steps, disabled at each end) editing `verticalIconStep` in vertical mode and `launcherIconStep` otherwise, Move bar `movableLauncher`, Remember tab `rememberPopoverTab`, plus Reset panel / Reset bar / Reset bar order (`resetLauncherOrder`) / Reset tabs); **Cache settings** (`wizCacheMinutes`, `sswCacheMinutes`); **Magma Pool** (Find pool time `magmaPoolCheck`, per-account times with a `nextPoolOpening` countdown, Forget; it reads the checker's cached `magmaAccount` and listens to `storage.onChanged`); **Backup** (Export cache `exportIncludeCache`, export/import via clipboard, file download, file pick → `importSettings`). The e2e suite asserts the group and title order. |
...
| `MagmaLog.vue` | The `magma` panel view, opened by the bar's Magma Pool button: the checker's `magmaLog` from storage.local (watched through `storage.onChanged`, so a check landing while it is open appears on top). A status line (the account's found time with a `nextPoolOpening` countdown, or what checking is waiting on), a Go to the pool link and Clear; one row per pool load with its result chip, local time and NST minute, what triggered it and for which account, the phrase it read, any saved time or note, and a two-line page excerpt. |
...
- Every drag handle (`.ns-grip`, `.ns-panel-head--grab`) needs `touch-action: none`. Without it a
  touch drag becomes a page pan, the browser fires `pointercancel`, and the handle only moves with a
  mouse — which is how the panel title bar stayed desktop-only while the popover grip worked on touch.
