...
| `PricePopover.vue` | The badge popover: a `v-menu` bound to `state.open`/`state.anchor`, showing spinner, error alert + `retry()`, or `PriceCard` + `HistoryTabs`. |
...
- The popover opens on `popoverTabs()[0]` — whichever tab was dragged to the front — and fetches
  it once the lookup lands (every tab but price needs the name/id it returns). Otherwise Wizard, SSW
  and TP fetch **only on tab selection**, searches being rate-limited; both paths go through
  `loadTab` so they cannot drift. Wizard and SSW results are reused for `RESULT_CACHE_MS` (15 min)
  per item; TP is fetched once per open.
