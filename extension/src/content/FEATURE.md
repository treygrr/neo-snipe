...
| `launcher.js` | The bottom-right bar: `addLauncher(onActivate)`, `setLauncherOpen`, `setLauncherDraggable`, `resetLauncherPosition`. Six children — `-grip` (the drag handle, leftmost, hidden when `movableLauncher` is off), `-main` (panel), `-sw`/`-ssw` (each opens its search panel; SSW hidden without Premium via `setLauncherPremium`), `-magma` (a link to the pool; `setMagmaState` off/idle/loading/found, `onMagmaClick`) and `-inv` (a plain link to `INVENTORY_URL`). `showLauncherNotice` pops a message above the bar. SVG glyphs are built as nodes, not `innerHTML`. Clamping and saved position via `lib/positions.js`. |
| `magma.js` | `startMagmaPool()`: the Magma Pool checker. While `magmaPoolCheck` is on and the logged-in account (`#flag_username` on /settings/account, cached 10 min as `magmaAccount`) has no time, loads the pool every 10 min on one clock shared by all tabs (`magmaLastCheck`, claimed before loading); being on the pool page counts as a check. On "open" it re-reads the account, saves the NST minute to `magmaPoolTimes`, and shows a notice. Reacts to `storage.onChanged`, so toggling or Forget updates every tab. |
