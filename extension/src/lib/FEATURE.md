...
| `launcher-size.js` | The bar's icon sizes, as steps: `ICON_STEPS` (5), `ICON_BASE_PX` (20), `ICON_STEP_PX` (4), `DEFAULT_ICON_STEP` (horizontal 1, vertical 4 — the 20px and 32px each bar shipped with), `iconPx(step)`, `cleanIconStep(value, fallback)`. Backs the `launcherIconStep`/`verticalIconStep` settings. |
