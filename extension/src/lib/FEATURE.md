...
| `magma.js` | **neopets.com/magma/pool.phtml + /settings/account.** `MAGMA_POOL_URL`, `MAGMA_CHECK_MS` (10 min, the window's length), `readPoolDetail(text)` → `{ state: open/closed/unknown, matched }` (the phrase that decided it) and `readPoolState(text)` (just the state), the check log (`MAGMA_LOG` key in storage.local, `MAGMA_LOG_MAX`, `withLogEntry`, `excerptOf(text, matched, max)`), `ACCOUNT_URL` (`/settings/account/`) + `isAccountPage` + `readAccountName(doc)` (`#flag_username` when the page's script has drawn it, else the header's "Welcome," profile link, else `appInsightsUserName` — a fetch gets only the latter two), `accountKey`, `poolTimeAt` (NST "HH:MM"), `nextPoolOpening`, `isPoolTime`, `cleanPoolTimes`. |
...
| `launcher-size.js` | The bar's icon sizes, as steps: `ICON_STEPS` (5), `ICON_BASE_PX` (20), `ICON_STEP_PX` (4), `DEFAULT_ICON_STEP` (horizontal 1, vertical 4 — the 20px and 32px each bar shipped with — and badge 1), `iconPx(step)`, `cleanIconStep(value, fallback)`, and the 🔍 badge's own sizes on the same steps: `BADGE_BASE_PX` (16), `badgePx(step)` (16–32px), `badgeGlyphPx(step)` (five eighths of it). Backs the `launcherIconStep`/`verticalIconStep`/`badgeIconStep` settings. |
| `tab-order.js` | Canonical `POPOVER_TABS` and order maths: `fullOrder(saved, known)`, `visibleOrder`, `moveInOrder`. |
| `settings-io.js` | Backup/restore: `EXPORT_VERSION`, `collectSettings()` (adds `cache` when `exportIncludeCache` is on), `toJson`, `parseExport(text, { now })` (validating — icon steps only as whole numbers 1–5 — throws `ImportError`; keeps only fresh current-version cache entries), `applyImport(parsed)` (cache entries added alongside, never replacing). |
