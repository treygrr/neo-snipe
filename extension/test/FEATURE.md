...
| `e2e.mjs` | Loads the real Chrome build (`NS_DIST` or `dist`) in a persistent context; ~320 checks over badge injection, the Magma Pool button already on the bar (and already found) before the account page answers, its click opening the check log (one row per pool load, the words read, the time saved, `aria-pressed`), popover tabs (price, TP, Wiz, Shops; order, open-on-first, a real tab drag that reorders and saves, Settings' Reset tabs restoring the shipped order, remembered tab, drag + viewport clamping), the launcher bar (its order; the Favourites/Dailies/Food Club/Settings buttons each opening their titled view, switching without a second panel, closing on a second click, and marking the open view's button with `aria-pressed`; the collapse arrow hiding all but grip and arrow with `aria-expanded` to match, restoring exactly what showed, working from Enter/Space with focus kept, a 400px window, a moved bar not drifting over repeated toggles and re-clamped on expand without saving, and starting expanded after a reload), the bar's icon size (Settings' −/+ stepping the horizontal bar from 20px to 36px glyphs live, disabled at each end, saved as `launcherIconStep`, exported with `verticalIconStep` and kept on reload; in vertical mode the same control stepping `verticalIconStep` down to 30px buttons around 20px glyphs without touching the horizontal size), the vertical bar (docked against the page edge and tucked behind its caret on load, 42px buttons around 32px glyphs, the caret unfolding it in place and folding it back to where it was docked, a drag snapping it to the left edge and saving that, a reload keeping the side, and turning it off laying the bar back down), reordering the bar (a button dragged past two others lands there without being pressed, the order saved as `launcherOrder` and kept on reload, Settings' Reset bar order restoring the shipped order) and its inventory link, the Quest Log panel (read on open, Fish, Spin and Read runners, Claim, each request counted against a small model of the server) and the bar's ready-to-claim count, the Purchase an Item helper (plan saved, shop opened with its cheapest item marked, haggle offer filled in, buy counted once, never a haggle sent), Customise a Pet (wardrobe loaded, one wearable added with nothing dropped, the original outfit saved back), the `#npanchor` rewrite, the wizard search panels (suggestions on click, outer search button, clear, sorts, layout budget, cross-wizard fold-out), panel, favourites, dailies groups, Food Club (loaded when its view opens) place/refuse, settings export/import, Premium gating, toolbar button (opens Favourites at the top), hover-only, and the Jelly-Neo-offline path. |
| `magma.test.mjs` | `lib/magma.js`: open/closed/unknown from the pool page's own text (both guard spellings) and the phrase that decided it, the capped newest-first check log and its excerpts, the NST minute recorded and the next opening across DST, the account from `#flag_username` or, on the settings page as fetched (no rendered div), the header's Welcome link / `appInsightsUserName`, and `cleanPoolTimes` on malformed imports. |
...
| `settings-io.test.mjs` | `toJson`/`parseExport`/`ImportError`/`EXPORT_VERSION` version rules (a retired `panelTabOrder`, `movablePanel` or `movableTabs` in an old export is ignored; `popoverTabOrder` and `launcherOrder` import, only as lists of ids; `launcherIconStep`/`verticalIconStep` only as whole steps 1–5), the `exportIncludeCache` switch and cached entries in a backup (current version, inside their day, well-formed, capped), plus `DAILIES`/`DAILY_COUNT`/`dailiesFor`/`isPremiumDaily` premium filtering from `dailies.js`. |
...
| `launcher-size.test.mjs` | `lib/launcher-size.js`: step 1 is 20px and each step to 5 adds 4px, each bar's default is the size it shipped with (and matches `DEFAULTS`), and `cleanIconStep` keeps only whole steps 1–5. |
