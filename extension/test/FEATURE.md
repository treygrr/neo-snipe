...
| `e2e.mjs` | Loads the real Chrome build (`NS_DIST` or `dist`) in a persistent context; ~157 checks over badge injection, popover tabs (price, TP, Wiz, Shops; order, open-on-first, remembered tab, drag + viewport clamping), panel, favourites, dailies groups, Food Club place/refuse, settings export/import, Premium gating, toolbar button, hover-only, and the Jelly-Neo-offline path. |
| `detect.test.mjs` | Runs `src/content/detect.js` inside Chromium against each surface fixture: `findItemElements`, `describeItem`, `itemImageUrl`, `itemNameFor`, `imageHashOf`; asserts no `obj_info_id` leaks. |
