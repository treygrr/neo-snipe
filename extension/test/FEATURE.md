...
| `e2e.mjs` | Loads the real Chrome build (`NS_DIST` or `dist`) in a persistent context; ~145 checks over badge injection, popover tabs (price, TP, Wiz, Shops; order + open-on-first), panel, favourites, dailies groups, Food Club place/refuse, settings export/import, Premium gating, toolbar button, hover-only, and the Jelly-Neo-offline path. |
